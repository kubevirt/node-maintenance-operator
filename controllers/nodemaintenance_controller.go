...
func init() {
	logrus.SetFormatter(&logrus.TextFormatter{TimestampFormat: "2006-01-02 15:04:05.000000", FullTimestamp: true})
}
...
var _ reconcile.Reconciler = &NodeMaintenanceReconciler{}
...
func (r *NodeMaintenanceReconciler) checkLeaseSupported() error {
	...
	if err != nil {
		logrus.Errorf("Failed to check for lease support %v", err)
		return err
	}
...
func (r *NodeMaintenanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	reqLogger := logrus.WithFields(logrus.Fields{"req.Namespace": req.Namespace, "req.Name": req.Name})
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			reqLogger.Infof("NodeMaintenance Object: %s Deleted ", req.NamespacedName)
	...
	if err != nil && updateOwnedLeaseFailed {
		...
		if instance.Status.ErrorOnLeaseCount > MaxAllowedErrorToUpdateOwnedLease {
			logrus.Info("can't extend owned lease. uncordon for now")
...
func setOwnerRefToNode(instance *nodemaintenancev1beta1.NodeMaintenance, node *corev1.Node) {
	...
	logrus.Info("setting owner ref to node")
...
func (r *NodeMaintenanceReconciler) obtainLease(node *corev1.Node) (bool, error) {
	...
	logrus.Info("Lease object supported, obtaining lease")
	lease, needUpdate, err := createOrGetExistingLease(r.Client, node, LeaseDuration)

	if err != nil {
		logrus.Errorf("failed to create or get existing lease error=%v", err)
		return false, err
	}

	if needUpdate {

		logrus.Info("update lease")
...
func (r *NodeMaintenanceReconciler) fetchNode(nodeName string) (*corev1.Node, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		logrus.Errorf("Node: %s cannot be found. Error: %v", nodeName, err)
		return nil, err
	} else if err != nil {
		logrus.Errorf("Failed to get Node %s: %v\n", nodeName, err)
		return nil, err
	}
	return node, nil
}
...
func (r *NodeMaintenanceReconciler) onReconcileErrorWithRequeue(nm *nodemaintenancev1beta1.NodeMaintenance, err error, duration *time.Duration) (reconcile.Result, error) {
	...
	if updateErr != nil {
		logrus.Errorf("Failed to update NodeMaintenance with \"Failed\" status. Error: %v", updateErr)
	}
	if duration != nil {
		logrus.Infof(FixedDurationReconcileLog)
		return reconcile.Result{RequeueAfter: *duration}, nil
	}
	logrus.Infof("Reconciling with exponential duration")
	return reconcile.Result{}, err
}
